    ...
    if (canvasRef.current) {
      ...
      if (ctx) {
        ...
        snake.forEach(([x, y], index) => {
          if (index === 0) {
            // Head of the snake
            ctx.fillStyle = "#6B8E23"; // dark green color for the head
            ctx.beginPath();
            ctx.arc(x + 0.5, y + 0.5, 0.5, 0, Math.PI * 2); // Draw head as a circle
            ctx.fill();

            // Draw eyes on the head
            ctx.fillStyle = "#fff"; // white for the eyes
            ctx.beginPath();
            ctx.arc(x + 0.7, y + 0.3, 0.1, 0, Math.PI * 2); // Right eye
            ctx.arc(x + 0.3, y + 0.3, 0.1, 0, Math.PI * 2); // Left eye
            ctx.fill();
          } else if (index === snake.length - 1) {
            // Tail of the snake
            ctx.fillStyle = "#9ACD32"; // lighter green color for the tail
            ctx.beginPath();
